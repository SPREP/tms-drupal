...
class Metar {
  ...
  private $weather_intensity_codes =
  [
    '-'  => 'light',
    '+'  => 'strong',
    'VC' => 'in the vicinity',
  ...
  private $runway_deposits_extent_codes =
  [
    '1' => 'from 10% or less',
    '2' => 'from 11% to 25%',
    '5' => 'from 26% to 50%',
  ...
  private $runway_deposits_depth_codes =
  [
    '00' => 'less than 1 mm',
    '92' => '10 cm',
    '93' => '15 cm',
  ...
  private function set_observed_date($time_utc) {
    ...
    // Or "D M j, H:i T".
    $this->set_result_value('observed_date', date('r', $local));
  ...
  private function get_visibility($part) {
    ...
    if ($found[1] == 'CAVOK' or $found[1] == '9999') {
      ...
      if ($found[1] == 'CAVOK') {
        ...
        // Can skip the next 4 methods: visibility_min, runway_vr, present_weather, clouds.
        $this->method += 4;
      }
    }
    elseif ($found[1] == '////') {
  ...
  private function get_clouds($part) {
    ...
    elseif (isset($found[5]) and !empty($found[5])) {
      ...
      $observed['height'] = $this->convert_distance($found[5] * 100, 'FT');
  ...
  private function get_pressure($part) {
    ...
    // Convert to in Hg.
    $this->set_result_value('barometer_in', round(0.02953 * $pressure, 2));
  ...
  private function get_wind_shear($part) {
    ...
    if (implode(' ', array_slice($this->raw_parts, $this->part, 2)) == 'ALL RWY') {
      ...
      // Can skip neext parts with ALL and RWY records.
      $this->part += 2;
    }
  ...
  private function get_trends($part) {
    ...
    if (empty($raw_parts)) {
      if ($trend['flag'] != 'CNL' and $trend['flag'] != 'NIL') {
        // Return pointer to previous part.
        $this->part--;
  ...
  private function calculate_wind_chill($temperature_f) {
    if ($temperature_f < 51 and $this->result['wind_speed'] != 0) {
      // Convert m/s to mi/h.
      $windspeed = round(2.23694 * $this->result['wind_speed']);
